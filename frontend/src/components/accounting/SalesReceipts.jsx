...
import {
  getSalesReceipts, createSalesReceipt, voidSalesReceipt,
  getBuyers, getBankAccounts, getActiveFlocks,
} from '../../api/accounting'
...
const emptyLine = () => ({ description: '', quantity: '', rate: '', amount: '', flock_id: '' })

const initialForm = () => ({
  customer_name: '', customer_id: '', receipt_date: today(),
  payment_method: 'Check', deposit_to: '', memo: '',
})
...
export default function SalesReceipts() {
  ...
  const [receiptPrefix, setReceiptPrefix] = useState('SR-')
  ...
  const loadReceipts = async () => {
    ...
    try {
      ...
      const res = await getSalesReceipts(params)
  ...
  const loadFormData = async () => {
    try {
      const [buyerRes, bankRes, flockRes, settingsRes] = await Promise.all([
        getBuyers(), getBankAccounts(), getActiveFlocks(), getSettings(),
      ])
      ...
      const prefix = s.sales_receipt_prefix?.value || 'SR-'
      const num = s.sales_receipt_next_number?.value || ''
  ...
  const handleSave = async (andNew) => {
    ...
    setSubmitting(true)
    try {
      const receiptNumber = nextNumber ? `${receiptPrefix}${nextNumber}` : `SR-${Date.now()}`
      const payload = {
      ...
      showToast('Sales receipt saved successfully')

      if (nextNumber) {
        const newNum = String(parseInt(nextNumber, 10) + 1)
        setNextNumber(newNum)
        try { await updateSettings({ sales_receipt_next_number: newNum }) } catch {}
      }
      ...
      showToast(err.response?.data?.detail || 'Error saving sales receipt', 'error')
  ...
  const handleVoid = async (receipt) => {
    if (!confirm(`Void sales receipt ${receipt.receipt_number || '#' + receipt.id}?`)) return
    try {
      await voidSalesReceipt(receipt.id)
      showToast('Sales receipt voided')
      loadReceipts()
    } catch (err) {
      showToast(err.response?.data?.detail || 'Error voiding sales receipt', 'error')
    }
  }
          ...
          <div style={{ display: 'flex', gap: 16, marginBottom: 10 }}>
            ...
            <div style={{ flex: '0 0 58%' }}>
              ...
              <input className="glass-input text-sm" type="date" value={form.receipt_date}
                onChange={e => updateField('receipt_date', e.target.value)} style={{ marginBottom: 6 }} />
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ marginBottom: 6 }}>
                <label style={{ fontSize: '8pt', color: '#999', display: 'block', marginBottom: 2 }}>Receipt #</label>
                <input className="glass-input text-sm" value={nextNumber ? `${receiptPrefix}${nextNumber}` : 'Auto'} readOnly
              ...
              <input className="glass-input text-sm" value={form.memo}
                ...
                placeholder="e.g. Weekly egg pickup - Route A" />
            </div>
          </div>
                  ...
                  <tr key={idx}>
                    <td>
                      <input className="glass-input text-sm" value={line.description}
                        ...
                        placeholder="e.g. 30 cases Grade A Large" />
                    </td>
                    <td>
    ...
    <div>
      ...
      <div className="glass-card p-4 m-2">
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
          <h2 style={{ fontSize: '14pt', fontWeight: 700, margin: 0 }}>Sales Receipts &mdash; Cash Egg Sales</h2>
          <button className="glass-button-primary text-sm" onClick={openNewReceipt}>+ New Sales Receipt</button>
        </div>
        ...
        {/* Table */}
        {loading ? (
          <p style={{ textAlign: 'center', padding: 20, color: '#999' }}>Loading sales receipts...</p>
        ) : receipts.length === 0 ? (
          <p style={{ textAlign: 'center', padding: 20, color: '#999' }}>No sales receipts found. Click "New Sales Receipt" to record a cash egg sale.</p>
        ) : (
          <table className="glass-table w-full">
            <thead>
              <tr>
            ...
            <tbody>
              {receipts.map(r => {
                ...
                return (
                  <tr key={r.id}>
                    <td style={{ fontWeight: 600 }}>{r.receipt_number || `SR-${r.id}`}</td>
