...
import { Bird, Egg, TrendingUp, DollarSign, Building2, Warehouse, AlertTriangle, AlertCircle, Info, Activity } from 'lucide-react'
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3 mb-6">
        {[
          { label: 'Active Flocks', value: stats.active_flocks, icon: Bird, color: 'text-lvf-accent' },
          { label: 'Total Birds', value: stats.total_birds.toLocaleString(), icon: Bird, color: 'text-lvf-success' },
          { label: 'Production %', value: `${stats.avg_production_pct}%`, icon: TrendingUp, color: stats.avg_production_pct >= 80 ? 'text-lvf-success' : stats.avg_production_pct >= 60 ? 'text-lvf-warning' : 'text-lvf-danger' },
          { label: 'Egg Inventory', value: `${stats.total_egg_skids || 0} skids`, icon: Egg, color: 'text-lvf-accent2' },
          { label: 'Growers', value: stats.active_growers, icon: Building2, color: 'text-lvf-text' },
          { label: 'Barns', value: stats.active_barns, icon: Warehouse, color: 'text-lvf-text' },
        ...
        <div className="glass-card p-5">
          ...
          {alerts.length > 0 ? (
            <div className="space-y-2">
              {alerts.map((alert, i) => (
                <div key={i} className={`p-3 rounded-xl border ${alertColor(alert.type)}`}>
          ...
          <div className="mt-4 pt-4 border-t border-lvf-border/30">
            <div className="flex items-center gap-2 mb-2">
      ...
      <div className="glass-card p-5">
        ...
        {activity.length > 0 ? (
          <div className="space-y-2">
            {activity.map((item, i) => (
              <div key={i} className="flex items-center justify-between py-2 border-b border-lvf-border/20 last:border-0">
